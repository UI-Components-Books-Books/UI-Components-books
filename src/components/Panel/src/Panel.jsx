...
export const Panel = ({
   ...
   * @returns {(Boolean)}
   */
  const validation = (id) => /:.*?:/g.test(id) ? isOpen === id : IdToSection[id - 1] === isOpen
   ...
   * @returns {number[]}
   */
  const normaliceSectionId = () => [...IdToSection].reduce((list, _, index) => [...list, index + 1], [])
   ...
   * @returns {number}
   */
  const getIndexById = (id) => IdToSection.indexOf(id) + 1
