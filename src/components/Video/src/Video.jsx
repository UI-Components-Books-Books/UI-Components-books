...
export const Video = ({
  ...
  function hourToString (timeSeconds) {
    ...
    if (secondsNumber <= 3600) {
      ...
      return `${getTime(timeSeconds).hours}:${getTime(timeSeconds).minutes}:${
        getTime(timeSeconds).seconds
      }`
    }
  }
  ...
  const handleArrowKeys = function (e) {
    const currentTime = Math.floor(
      refProgress.current.getAttribute('aria-valuenow')
    )
  ...
  useEffect(() => {
    localStorage.setItem(
      'ui-video',
      JSON.stringify({ caption: captions, volume: getValueVolume })
    )
      ...
      <div className={`${css['c-vid__container']} ${addClass}`} ref={refCont}>
        <div className={css['c-vid__wrapper']}>
          <video
            ...
            onClick={handlePlay}
            className={`${css['c-vid__video']} ${
              captions ? '' : css['no-captions']
            }`}
            {...(poster && { poster })}
          >
        ...
        <div className={css['c-vid__controls']}>
          ...
          <p className={css['c-vid__time']} aria-hidden='true'>
            <span>{getCurrentTime.string}</span>/
            <span>{getDurationVideo.string}</span>
          </p>
