...
export const Interpreter = ({ width, addClass, image, pathVideo, pathname }) => {
  ...
  return (
    <Draggable nodeRef={nodeRef} handle="strong" position={deltaPosition} onDrag={handleDrag}>
      <div
        ref={nodeRef}
        className={`${css.interpreter} ${addClass ?? ''} ${(showVideo1 || showVideo2) && css['interpreter-active']}`}>
        <div className={css['interpreter-dropdown']}>
          {((!showVideo1 && !showVideo2) ||
            ...
            <button
              {...(!hidden && !showVideo1 && !showVideo2 && { 'data-interpreter-primary-button': true })}
              onClick={() => {
                if (!hidden) {
              ...
              className={`${css['interpreter-btn']} ${css['interpreter-btn-switch']} ${
        ...
        {/* Contendor video */}
        <div className={css['c-vid-container']} display={showVideo1 || showVideo2 ? 'true' : 'false'}>
